...
client.on('ready', () => {
  console.log(`Amor Bot is has logged in succes. ${client.user.tag}!`);
});
...
client.on("message", msg => {
  ...
  if (küfür.some(word => msg.content.toLowerCase().includes(word)) ) {
    if (!msg.member.hasPermission("ADMINISTRATOR")) {
      msg.delete()
       msg.reply("Bu sunucuda küfürler **Amor** tarafından engellenmektedir! Küfür engeli Açık!").then(message => message.delete(3000));
    }
}
    }
});
    ...
    .setTitle(':warning: | İlk olarak sesli bir kanala giriş yapmanız gerek.'));
      ...
      .setTitle(':warning: | Şarkı başlatılamıyor. Lütfen mikrofonumu açınız.'));
		...
		for (const video of Object.values(videos)) {
			 ...
			 return msg.channel.sendEmbed(new Discord.RichEmbed)
      .setTitle(`**Amor | Oynatma Listesi: **${playlist.title}** Kuyruğa Eklendi!**`)
		} else {
			try {
         ...
         .setColor('RED'));
          msg.delete(5000)
            ...
            .setDescription(':warning: | **Şarkı Değeri Belirtmediğiniz İçin Seçim İptal Edilmiştir**.'));
					...
					return msg.channel.sendEmbed(new Discord.RichEmbed()
          .setColor('RED')
          .setDescription('Amor:(**Aradım Fakat Hiç Bir Sonuç Çıkmadı**'));
                }
            }
...
function play(guild, song) {
  ...
  .setTitle("**Amor | :microphone: Müzik Başladı**")
...
client.on("message", msg => {
    ...
    const embedlul = new Discord.RichEmbed()
      .setColor(0x00AE86)
      .setDescription("" + msg.author + "**Amor** Reklam koruması aktif! Sunucu da reklam yaptığın için yasaklanabilirsin!")
...
// Oto mesaj
client.on('message', msg => {
if (msg.content === 'canlıyayın') {
  msg.reply('Hey Kondrin20 Canlı Yayında! Twitch Link : https://www.twitch.tv/kondrin20 @everyone');
}
});
///////////////////////////////////
//otorol
